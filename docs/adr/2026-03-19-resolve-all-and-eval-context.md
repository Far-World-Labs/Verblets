...
| Rename `optionContext → evalContext` | "Context" unqualified should mean LLM prompt context in this codebase. The evaluation context for `policy` functions needs an explicit qualifier. `evalContext` is short, descriptive, and won't collide. | `resolveContext` (longer, and "resolve" already overloaded with the `resolve` function), `configContext` (ambiguous — could mean the config object itself), keeping `optionContext` (perpetuates the ambiguity) |
