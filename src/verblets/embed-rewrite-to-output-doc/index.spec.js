...
describe('embedRewriteToOutputDoc', () => {
  it('passes prompt and schema to callLlm', async () => {
    ...
    expect(options.modelOptions.response_format.json_schema.name).toBe('hyde_output_doc');
    expect(options.modelOptions.response_format.json_schema.schema.properties.value.type).toBe(
      'string'
    );
  });
