...
Use a logarithmic transformation to handle the wide input range.`;
  ...
  it(
    'should map star ratings to quality scores',
    async () => {
      const prompt = `
...
Mapping: Map the "stars" field linearly to the quality range. 1 star = 0, 5 stars = 100.`;
  ...
  it(
    'should evaluate multidimensional apartment desirability',
    async () => {
      const prompt = `
...
Weight location and space slightly higher, but consider that perfect scores in any two factors can compensate for weaknesses.`;
  ...
  it(
    'should apply complex objection-adjusted approval voting scale',
    async () => {
      const prompt = `
...
describe.skipIf(!isMediumBudget)('[medium] createScale examples', () => {
  ...
  it(
    ...
    async () => {
      ...
      const sentimentSpec = await scaleSpec(
        'Rate text sentiment from -1 (negative) to 1 (positive)'
      );
...
describe.skipIf(!isMediumBudget)('[medium] scaleSpec and applyScale examples', () => {
  ...
  it('should handle complex domain-specific scaling', { timeout: longTestTimeout }, async () => {
    const spec = await scaleSpec(`
