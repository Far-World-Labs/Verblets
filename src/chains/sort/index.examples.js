...
describe('sort examples', () => {
  it(
    ...
    async () => {
      const movieData = fs.readFileSync(
        path.join(__dirname, 'test-data', 'movies.txt'), 
        'utf8'
      );
      const movieSuggestions = movieData.trim().split('\n').filter(line => line.trim());
  ...
  it(
    'gift matching',
    async () => {
      const giftData = fs.readFileSync(
        path.join(__dirname, 'test-data', 'gift-ideas.txt'),
        'utf8'
      );
      const giftIdeas = giftData.trim().split('\n').filter(line => line.trim());
      
      const sorted = await sort(
        giftIdeas, 
        "for someone who gardens and likes animals",
        {
          chunkSize: 10,
          extremeK: 7,
          iterations: 1
