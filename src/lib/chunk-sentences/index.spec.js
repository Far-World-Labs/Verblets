...
runTable({
  ...
  examples: [
    ...
    },
    {
      ...
      inputs: { text: 'First sentence. Second sentence. Third sentence.', maxLen: 20 },
  ...
  process: ({ inputs }) => {
    const chunks = chunkSentences(inputs.text, inputs.maxLen);
    return {
      chunks,
      multipleChunks: chunks.length > 1,
      contentPreserved: chunks.join('') === (inputs.text ?? ''),
    };
  },
